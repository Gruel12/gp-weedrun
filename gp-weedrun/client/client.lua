...
CreateThread(function()
  RequestModel(`a_f_y_beach_01`)
    while not HasModelLoaded(`a_f_y_beach_01`) do
    Wait(1)
  end
    methboss = CreatePed(2, `a_f_y_beach_01`, 481.18, -591.21, 23.75, 299.77, false, false) -- change here the cords for the ped 
    SetPedFleeAttributes(methboss, 0, 0)
    SetPedDiesWhenInjured(methboss, false)
    TaskStartScenarioInPlace(methboss, "WORLD_HUMAN_HANG_OUT_STREET", 0, true)
...
--- Target Stuff
CreateThread(function()
    exports['qb-target']:AddTargetModel('a_f_y_beach_01', {
        options = {
            { 
                type = "client",
                event = "ps-methrun:client:start",
                icon = "fas fa-circle",
                label = "start Weed run ($10000)",
            },
            { 
                type = "client",
